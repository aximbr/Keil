...
int main(void){
  ...
  GPIO_PORTF_AHB_DATA_R |= LED_RED; 	//Turn ON Red LED (PF1) and Blue LED (PF2)
	GPIO_PORTF_AHB_DATA_R |= LED_BLUE;	//Turn ON Red LED (PF1) and Blue LED (PF2)
	//inicializa as duas tasks, com a mesma prioridade
	
	resultado = xTaskCreate(vBlinkRedLed, "Red Led", 100, NULL, 1, NULL);
	resultado = xTaskCreate(vBlinkBlueLed, "Blue Led", 100, NULL, 1, NULL);
