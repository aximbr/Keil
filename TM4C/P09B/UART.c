...
// Output: none
int UART_init (void){
  SYSCTL->RCGC1 |= SYSCTL_RCGC1_UART0;  // activate clock on UART0
	SYSCTL->RCGC2 |= SYSCTL_RCGC2_GPIOA;  // activate clock port A
  UART0->CTL &= ~UART_CTL_UARTEN;       // disable UART
  UART0->IBRD =27;                      // IBRD = int(50,000,000 / (16 * 115,200)) = int(27.1267)
  UART0->FBRD = 8;                      // FBRD = int(0.1267 * 64 + 0.5) = 8
...
unsigned char UART_InChar(void){
  ...
  return((unsigned char)(UART0->FR &0xFF));
}
