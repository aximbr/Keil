/*Etapa 13                                                        */
/*Projeto utizando 3 pinos de GPIO conectados ao módulo multifuncional 
...
#define LATCH_PIN    _BV(0)  //D4 <-> PB0
#define CLK_PIN      _BV(1)  //D7 <-> PB1
#define DATA_PIN     _BV(2)  //D8 <-> PB2

#define bitSet(_pin)    GPIOB->PSOR |=(_pin)
#define bitClear(_pin)  GPIOB->PCOR |=(_pin)
...
/* Initiate GPIO */
	...
	PORTB->PCR[0] |= PORT_PCR_MUX(1);  //Função de GPIO
	PORTB->PCR[1] |= PORT_PCR_MUX(1);  //Função de GPIO
	PORTB->PCR[2] |= PORT_PCR_MUX(1);  //Função de GPIO
	GPIOB->PDDR |= LATCH_PIN | CLK_PIN | DATA_PIN;   //Porta B como output (pinos 0, 1 e 2)
