/**
  ...
  * the Blue Led on PD.1	turns on
	* After reaches Timeout, the Match value is incremented.
...
void Timer0_Init(void){
	...
	*/
	  ...
	  TPM0_C0SC |= TPM_CnSC_CHIE(1);   //enable interrupt when match
	  TPM0_C0V  = c0value;             //change every timeout event
...
void TPM0_IRQHandler(void)
{
	if ( TPM0->SC & TPM_SC_TOF_MASK )        //we have a time-out
		   {GPIOD->PCOR |= (1UL << 1);         //clear PD.1 turn-on Blue LED
				 if (c0value < 0xFFF0) c0value= c0value+0xF;
				 if (c0value == 0xFFFF) c0value=0;
				TPM0_C0V = c0value;
			  TPM0->SC |= TPM_SC_TOF(1);}			   //clear TOF Bit 
	else if (TPM0_C0SC & TPM_CnSC_CHF_MASK)  //we have a match
	     {GPIOD->PSOR |= (1UL << 1);         //set PD.1 turn-off Blue LED
				TPM0_C0SC |= TPM_CnSC_CHF_MASK;}   //clear CHF bit
}
