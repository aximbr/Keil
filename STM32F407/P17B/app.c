...
void dac_out( GPIO_TypeDef *GPIOx, uint16_t mask, uint16_t val);
void dac_init(void);
...
int main(void){
  ...
  SystemInit();   //STM32F407 Board Initialization
...
static void App_task_dac_out (void *p_arg){
	...
	while(DEF_TRUE){
		dac_out(GPIOA, BIT_MASK, wave[++pos & BIT_MASK]);
...
void dac_init(void){
	...
	myGPIO_init.Pin = GPIO_PIN_0 |GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5;
	myGPIO_init.Mode = GPIO_MODE_OUTPUT_PP;
	myGPIO_init.Speed = GPIO_SPEED_FREQ_LOW;
	myGPIO_init.Pull = GPIO_PULLUP;
