...
int main(void)
{
  ...
  RCC_Configuration();
	...
	while(1) {
		  /*The CCR1Val is incremented by IRQ Handler when the counter reaches zero (update event)*/
      if (CCR1Val < 1125) {TIM1->CCR1 = CCR1Val;}
			else  {CCR1Val =  0;}  //restart CCR1Val
	}
...
void NVIC_Configuration(void)
{
  ...
  /* Enable the Compare Match global Interrupt */
  ...
  NVIC_Init(&NVIC_InitStructure);
	...
	/* Enable the Update global Interrupt */
